...
from safety_logger import SafetyEventType, SafetyLogger
...
class TestTickReplayWithAutoGreen:
    ...
    def test_tick_stream_simulation(self):
        ...
        orders = [
            MockOrder(
                {"auto_green": True, "placed_at": base_time - 3.0, "simulation": False}
            ),
            MockOrder(
                {"auto_green": True, "placed_at": base_time - 1.0, "simulation": False}
            ),
            MockOrder(
                {"auto_green": False, "placed_at": base_time - 5.0, "simulation": False}
            ),
        ]
...
class TestSafetyLoggerPerformance:
    ...
    def test_1000_log_entries_performance(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "safety_logger.SafetyLogger._get_log_directory", lambda self: tmp_path
        )
        ...
        for i in range(1000):
            ...
            logger.log_event(
                event_type, f"Test message {i}", {"iteration": i, "value": f"data_{i}"}
            )
    ...
    def test_concurrent_logging(self, tmp_path, monkeypatch):
        ...
        assert (
            elapsed < 3.0
        ), f"1000 log entries concorrenti in {elapsed:.3f}s (max 3.0s)"
    ...
    def test_full_workflow_stress(self, tmp_path, monkeypatch):
        ...
        for i in range(100):
            ...
            if not should_auto_green(order, "OPEN"):
                logger.log_auto_green_denied("Check failed", f"ORD_{i}", "OPEN")
