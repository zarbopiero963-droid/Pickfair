...
class TestGuardrailAutoGreen(unittest.TestCase):
    ...
    def test_grace_period_expired(self):
        ...
        bet_id = "BET456"
        self.guardrail.register_order_for_auto_green(
            bet_id, placed_at=time.time() - 5.0
        )
...
class TestGuardrailErrorState(unittest.TestCase):
    ...
    def setUp(self):
        config = GuardrailConfig(
            consecutive_error_limit=2, cooldown_after_error_sec=1.0
        )
        self.guardrail = AIGuardrail(config)
...
class TestGuardrailFullCheck(unittest.TestCase):
    ...
    def test_full_check_pass(self):
        ...
        result = self.guardrail.full_check(
            market_type="MATCH_ODDS", tick_count=30, wom_confidence=0.6, volatility=0.3
        )
    ...
    def test_full_check_warning(self):
        ...
        result = self.guardrail.full_check(
            market_type="MATCH_ODDS", tick_count=30, wom_confidence=0.6, volatility=0.85
        )
    ...
    def test_full_check_insufficient_data_blocks(self):
        ...
        result = self.guardrail.full_check(
            market_type="MATCH_ODDS", tick_count=7, wom_confidence=0.6, volatility=0.3
        )
