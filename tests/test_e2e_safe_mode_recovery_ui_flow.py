...
class DummyBus:
    def __init__(self):
        self.subscribers = {}
        self.events = []
        self.ui_state = {"safe_mode": False, "safe_mode_reason": None}
    ...
    def publish(self, event_name, payload):
        ...
        if event_name == "SAFE_MODE_TRIGGER":
            self.ui_state["safe_mode"] = True
            self.ui_state["safe_mode_reason"] = payload.get("reason")
...
def test_e2e_safe_mode_recovery_ui_flow():
    bus = DummyBus()
    db = DummyDB()
    ...
    assert bus.ui_state["safe_mode"] is True
    assert bus.ui_state["safe_mode_reason"] == "Circuit Breaker"
