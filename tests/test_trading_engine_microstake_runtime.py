...
def test_microstake_boundary_values_are_detected_correctly():
    engine, _, _, _ = _make_engine(HappyMicroClient())
...
def test_microstake_success_payload_has_expected_flags_and_status():
    engine, bus, db, client = _make_engine(HappyMicroClient())
...
def test_microstake_creates_pending_saga_before_resolution():
    engine, _, db, _ = _make_engine(HappyMicroClient())
...
def test_execute_micro_stake_rejects_too_small_requested_stake():
    engine, _, _, _ = _make_engine(CancelFailClient())
...
def test_execute_micro_stake_cancel_failure_raises_runtime_error():
    client = CancelFailClient()
    engine, _, _, _ = _make_engine(client)
