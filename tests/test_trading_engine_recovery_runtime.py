...
def test_recovery_reconciles_successful_rows():
    pending = [
        {
            "customer_ref": "REC-1",
            "market_id": "1.111",
            "selection_id": 11,
            "raw_payload": json.dumps(_payload()),
            "status": "PENDING",
        },
        {
            "customer_ref": "REC-2",
...
def test_recovery_marks_failed_when_api_rejects():
    pending = [
        {
            "customer_ref": "FAIL-1",
            "market_id": "1.333",
...
def test_recovery_skips_when_no_pending_rows():
    bus = DummyBus()
    db = DummyDB(pending=[])
    client = MatchingClient()
    engine = TradingEngine(bus, db, lambda: client, DummyExecutor())
