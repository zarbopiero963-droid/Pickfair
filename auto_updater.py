...
class UpdateDialog:
    ...
    def __init__(self, parent, update_info):
        ...
        title_label = ttk.Label(
            frame, text="Nuovo Aggiornamento!", font=("Segoe UI", 14, "bold")
        )
        ...
        notes_text.insert(
            "1.0", update_info.get("release_notes", "Nessuna nota disponibile")
        )
        ...
        self.progress_bar = ttk.Progressbar(
            self.progress_frame, mode="determinate", length=380
        )
        ...
        self.manual_btn = ttk.Button(
            self.btn_frame, text="Scarica Manualmente", command=self._manual_download
        )
        self.manual_btn.pack(side=tk.LEFT, padx=5)

        self.later_btn = ttk.Button(self.btn_frame, text="Dopo", command=self._later)
        ...
        def do_download():
            def progress_callback(downloaded, total):
                if total > 0:
                    ...
                    self.dialog.after(
                        0, lambda: self._update_progress(percent, mb_down, mb_total)
                    )
    ...
    def _download_failed(self):
        ...
        self.messagebox.showerror(
            "Errore", "Download fallito. Prova il download manuale."
        )
