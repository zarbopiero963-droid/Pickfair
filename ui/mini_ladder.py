...
from safety_logger import evaluate_runner_liquidity
...
class MiniLadder(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        self.liquidity_indicator = ctk.CTkLabel(
            header, text="", width=12, height=12, corner_radius=6, fg_color="#4CAF50"
        )
        ...
        # Container prezzi
        prices_frame = ctk.CTkFrame(
            self, fg_color=COLORS.get("bg_secondary", "#2b2b2b")
        )
    ...
    def update_prices(self, runner: Dict):
        ...
        for i, lbl in enumerate(self.back_labels):
            if i < len(back_ladder):
                ...
                if price == best_back:
                    lbl.configure(
                        fg_color=COLORS.get("back", "#1e88e5"), text_color="white"
                    )
                else:
                    lbl.configure(
                        fg_color=COLORS.get("back_bg", "#1e3a5f"),
        ...
        for i, lbl in enumerate(self.lay_labels):
            if i < len(lay_ladder):
                ...
                if price == best_lay:
                    lbl.configure(
                        fg_color=COLORS.get("lay", "#e5399b"), text_color="white"
                    )
                else:
                    lbl.configure(
                        fg_color=COLORS.get("lay_bg", "#5f1e3a"),
    ...
    def _on_back_click(self, index: int):
        ...
        if index < len(back_ladder):
            ...
            self.on_price_click(self.runner.get("selectionId"), "BACK", price)
    ...
    def _on_lay_click(self, index: int):
        ...
        if index < len(lay_ladder):
            ...
            self.on_price_click(self.runner.get("selectionId"), "LAY", price)
    ...
    def set_edge_badge(self, edge_score: float, confidence: float):
        ...
        if not hasattr(self, "_edge_badge"):
            self._edge_badge = ctk.CTkLabel(
                self, text="", font=("Roboto", 9), corner_radius=3, width=60, height=18
            )
...
class OneClickLadder(MiniLadder):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            parent, runner, on_price_click=self._handle_one_click, **kwargs
        )
...
class LiveMiniLadder(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        for idx, sel in enumerate(self.selections):
            ...
            badge_color = (
                COLORS.get("back", "#1e88e5")
                if side == "BACK"
                else COLORS.get("lay", "#e5399b")
            )
    ...
    def _refresh(self):
        ...
        if hasattr(self.controller, "wom_engine") and self.controller.wom_engine:
            try:
                edge_scores = self.controller.wom_engine.get_ai_edge_score(
                    self.selections
                )
            except Exception:
                pass
        ...
        for sel in self.selections:
            ...
            if edge:
                side = (
                    edge.suggested_side
                    if hasattr(edge, "suggested_side")
                    else edge.get("side", "BACK")
                )
            else:
                side = sel.get("side", "BACK")
            ...
            pnl_color = (
                COLORS.get("profit", "#4caf50")
                if pnl >= 0
                else COLORS.get("loss", "#f44336")
            )
            self.pnl_labels[sel_id].configure(
                text=f"P&L: {pnl:.2f}", text_color=pnl_color
            )

            if sel_id == best_sel_id:
                self.name_labels[sel_id].configure(
                    fg_color=COLORS.get("warning", "#ff9800"), corner_radius=3
                )
            else:
