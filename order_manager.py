...
def calculate_profit_delta(
    old_price: float, new_price: float, stake: float, side: str
) -> float:
    ...
    if side == "BACK":
        if normalized_best > current_price:
            if profit_delta < profit_threshold:
                return (
                    False,
                    f"Profit delta {profit_delta:.2f} < threshold {profit_threshold}",
                    profit_delta,
                )
        ...
        if normalized_best < current_price:
            if abs(profit_delta) < profit_threshold:
                return (
                    False,
                    f"Liability delta {abs(profit_delta):.2f} < threshold {profit_threshold}",
                    profit_delta,
                )
            return (
                True,
                f"LAY: -{ticks} tick, +{abs(profit_delta):.2f}EUR liability saved",
                profit_delta,
            )
        else:
...
class OrderManager:
    ...
    def smart_update_order(
        ...
        logger.info(
            f"[ORDER_MGR] {side} {current_bet_id}: {current_price} -> {normalized_price} ({reason})"
        )

        try:
            response = self.client.replace_orders(
                market_id=market_id, bet_id=current_bet_id, new_price=normalized_price
            )
            ...
            if response.get("status") == "SUCCESS":
                ...
                if reports:
                    ...
                    if new_bet_id and new_bet_id != current_bet_id:
                        ...
                        logger.info(
                            f"[ORDER_MGR] BetId changed: {current_bet_id} -> {new_bet_id}"
                        )
                    else:
    ...
    def _fallback_cancel_place(
        ...
        try:
            ...
            if place_result.get("status") == "SUCCESS":
                ...
                logger.info(
                    f"[ORDER_MGR] Fallback success: {original_bet_id} -> {new_bet_id}"
                )
            else:
    ...
    def _record_history(self, action: str, result: Dict):
        ...
        self.order_history.append(
            {"timestamp": time.time(), "action": action, **result}
        )
...
def batch_follow_orders(
    ...
    for order in orders:
        ...
        best_price = prices.get(
            "back" if side == "BACK" else "lay", order.get("price", 0)
        )
