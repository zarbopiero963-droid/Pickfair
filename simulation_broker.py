...
class SimulationBroker:
    ...
    def __init__(
        ...
        """
        actual_balance = (
            initial_balance if initial_balance is not None else SIM_INITIAL_BALANCE
        )
    ...
    def place_order(
        ...
        with self.lock:
            ...
            if price_ladder:
                ...
                final_price = price
                fills = (
                    [{"price": price, "size": matched_size}] if matched_size > 0 else []
                )
            ...
            logger.info(
                f"[SIM] {side} €{matched_size:.2f}/{size:.2f} @ {final_price:.2f} su {runner_name} -> {bet_id}"
            )
    ...
    def cancel_order(self, bet_id: str) -> dict:
        ...
        with self.lock:
            ...
            if order.matched == 0:
                ...
                logger.info(
                    f"[SIM] Ordine {bet_id} cancellato completamente (size={size_to_cancel:.2f})"
                )
                return {
                    "success": True,
            ...
            if order.matched > 0 and order.size_remaining > 0:
                ...
                logger.info(
                    f"[SIM] Ordine {bet_id} parziale: cancellato €{size_to_cancel:.2f}, matched €{order.matched:.2f} resta attivo"
                )
                return {
                    "success": True,
                    "sizeCancelled": size_to_cancel,
        ...
        with self.lock:
            ...
            for order in self.orders.values():
                ...
                if unmatched > 0:
                    ...
                    logger.debug(
                        f"[SIM] Rilasciata exposure non matchata €{unmatched:.2f} per {order.bet_id}"
                    )
...
class BookOptimizer:
    ...
    def optimize(
        ...
        for _iteration in range(max_iterations):
            ...
            unclamped_prob_sum = sum(
                1 / s["price"] for s in unclamped if s.get("price", 0) > 1
            )
        ...
        logger.info(
            f"[BOOK OPT] Ribilanciato: {current_book:.1f}% -> {final_book:.1f}% (target {target_book:.1f}%)"
        )
        return result
    ...
    def validate_stakes(self, selections: list[dict]) -> list[str]:
        ...
        for s in selections:
            ...
            if 0 < stake < self.min_stake:
                errors.append(
                    f"{s.get('runnerName', 'Runner')}: €{stake:.2f} < min €{self.min_stake:.2f}"
                )
        return errors
