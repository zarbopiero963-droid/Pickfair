...
class DutchingController:
    ...
    def __init__(self, broker, pnl_engine, simulation: bool = False):
        """
        Args:
    ...
    def submit_dutching(
        ...
        if ai_enabled:
            if not self.market_validator.is_dutching_ready(market_type):
                self.safety_logger.log_ai_blocked(
                    market_type=market_type, reason="MARKET_NOT_COMPATIBLE"
                )
                raise ValueError(
        ...
        try:
            if mode == "MIXED":
                ...
                results, profit, implied_prob = calculate_dutching_stakes(
                    selections, total_stake, bet_type=mode, commission=commission
                )
        except Exception as e:
        ...
        logger.info(
            f"[CONTROLLER] Dutching {status}: {len(placed)} ordini, sim={self.simulation}"
        )
    ...
    def preflight_check(
        ...
        for sel in selections:
            ...
            if back_ladder and lay_ladder:
                ...
                if best_back > 0 and best_lay > 0:
                    ...
                    tick_size = (
                        0.02 if best_back < 2 else 0.05 if best_back < 4 else 0.1
                    )
        ...
        # Log preflight
        logger.info(
            f"[PREFLIGHT] valid={result.is_valid}, warnings={len(result.warnings)}, errors={len(result.errors)}"
        )
    ...
    def check_guardrail(
        self, market_type: str, selection_id: Optional[int] = None
    ) -> Dict:
        """
