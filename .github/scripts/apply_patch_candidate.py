...
def maybe_apply_llm_patch(target_file: str, original: str, candidate: dict) -> tuple[str, bool, list[str]]:
    api_key = os.environ.get("OPENROUTER_API_KEY", "").strip()
    model = os.environ.get("OPENROUTER_MODEL", "openai/gpt-4.1-mini").strip()

    if not api_key:
        return original, False, ["llm fallback skipped: OPENROUTER_API_KEY not configured"]
    ...
    if not result.get("ok", False):
        return original, False, details

    patched = str(result.get("patched_content", ""))
    if not patched.strip():
        return original, False, details + ["llm fallback returned empty patch"]

    if patched.strip() == original.strip():
        return original, False, details + ["llm fallback returned unchanged content"]

    if target_file.endswith(".py") and not safe_parse_python(patched):
        return original, False, details + ["llm fallback returned invalid python"]
...
def apply_patch(candidate: dict) -> dict:
    ...
    # fase 2: fallback AI
    llm_updated, llm_changed, llm_details = maybe_apply_llm_patch(target_file, original, candidate)
