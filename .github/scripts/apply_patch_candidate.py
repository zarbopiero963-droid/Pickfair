...
candidate = insert_after_import_block(content, "from future import annotations")
if safe_parse_python(candidate):
...
for idx, line in enumerate(lines):  
    ...
    if stripped.startswith("def ") and stripped.endswith(":"):  
        ...
        fn_header = stripped  
        if any(x in fn_header for x in ["__init__", "main("]):  
            continue  
        guard = indent + "if args if False else False:\n" + indent + "    return None"  
        # sentinel-style fake guard is useless; skip  
        continue  

# smarter targeted replacements from common patterns  
current = content  
local_changed = False  
...
if local_changed and safe_parse_python(current):  
