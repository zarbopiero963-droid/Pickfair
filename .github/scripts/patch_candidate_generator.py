...
def is_test_target(path_str: str) -> bool:
    path_str = normalize_path(path_str).lower()
    return path_str.startswith("tests/") and path_str.endswith(".py")
...
def is_test_failure(item: dict) -> bool:
    return str(item.get("issue_type", "")).strip() == "test_failure"


def runtime_score(item: dict, cto_item: dict) -> tuple:
    ...
    return (
        ...
        0 if classification == "AUTO_FIX_SAFE" else 1,
...
def general_score(item: dict, cto_item: dict) -> tuple:
    target_file = normalize_path(item.get("target_file", ""))
    cto_priority = str(cto_item.get("priority", "P2")).strip().upper()
    classification = str(item.get("classification", "")).strip()
    ...
    return (
        0 if cto_priority == "P0" else 1 if cto_priority == "P1" else 2,
        0 if classification == "AUTO_FIX_SAFE" else 1,
        0 if issue_type == "runtime_failure" else 1,
        0 if issue_type == "lint_failure" else 1,
        0 if is_runtime_target(target_file) else 1,
        1 if is_guardrail_test(target_file) else 0,
        target_file,
    )
...
def choose_candidate(contexts):
    ...
    for item in classified_items:
        ...
        r_score = runtime_score(item, cto_item)
        g_score = general_score(item, cto_item)

        if is_runtime_target(target_file):
            if is_runtime_failure(item):
                if is_auto_fix_safe(item):
                    runtime_fail_safe.append((r_score, item, cto_item))
                elif is_auto_fix_review(item):
                    runtime_fail_review.append((r_score, item, cto_item))

            if is_lint_failure(item) and is_auto_fix_safe(item):
                lint_safe.append((r_score, item, cto_item))

        if is_test_target(target_file) and not is_guardrail_test(target_file) and is_auto_fix_safe(item):
            if is_test_failure(item) or str(item.get("issue_type", "")).strip() == "missing_nominal_test":
                test_safe.append((g_score, item, cto_item))

        if is_auto_fix_safe(item):
            safe_candidates.append((g_score, item, cto_item))
        elif is_auto_fix_review(item):
            review_candidates.append((g_score, item, cto_item))
    ...
    if test_safe:
        test_safe.sort(key=lambda x: x[0])
        _, item, cto_item = test_safe[0]
        return candidate_from_classified_item(item, cto_item, "safe_test_fix")
    ...
    for item in fix_context.get("fix_contexts", []) or []:
        ...
        if is_runtime_target(target_file) or is_generated_test(target_file) or is_test_target(target_file):
            cto_item = cto_map.get(target_file, {})
            return candidate_from_classified_item(
