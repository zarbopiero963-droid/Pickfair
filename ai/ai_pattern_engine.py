...
class AIPatternEngine:
    ...
    def __init__(
        self, wom_back_threshold: float = 0.55, wom_lay_threshold: float = 0.45
    ):
        """
        Args:
    ...
    def decide(self, selections: List[Dict]) -> Dict[int, str]:
        ...
        if len(sides) == 1 and len(selections) > 1:
            ...
            if best_candidate:
                ...
                logger.info(
                    f"[AI] Forzato {best_candidate} a {decisions[best_candidate]} per mixed"
                )
    ...
    def get_enhanced_analysis(
        ...
        for idx, sel in enumerate(selections):
            ...
            hist_result = (
                wom_engine.calculate_wom(selection_id) if selection_id else None
            )
            ...
            if hist_result:
                ...
                edge_score = (
                    hist_result.edge_score * 0.7 + (instant_wom - 0.5) * 2 * 0.3
                )
