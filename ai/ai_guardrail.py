...
class AIGuardrail:
    ...
    def check_order_rate(self) -> tuple[bool, Optional[BlockReason]]:
        ...
        with self._lock:
            ...
            recent_orders = [
                o for o in self.state.order_history if o.timestamp > one_minute_ago
            ]
        ...
        with self._lock:
            ...
            if success:
                ...
                if self.state.consecutive_errors >= self.config.consecutive_error_limit:
                    ...
                    logger.warning(
                        f"[GUARDRAIL] Consecutive errors: {self.state.consecutive_errors}, blocked for {self.config.cooldown_after_error_sec}s"
                    )
    ...
    def get_status(self) -> Dict:
        ...
        with self._lock:
            now = time.time()
            recent_orders = [
                o for o in self.state.order_history if o.timestamp > now - 60.0
            ]
            ...
            return {
                ...
                "blocked_until": (
                    self.state.blocked_until if self.state.blocked_until > now else 0
                ),
    ...
    def set_manual_block(self, blocked: bool):
        ...
        with self._lock:
            if blocked:
                ...
                self.state.block_reasons = [
                    r for r in self.state.block_reasons if r != BlockReason.MANUAL_BLOCK
                ]
