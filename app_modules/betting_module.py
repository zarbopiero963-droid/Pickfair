...
class BettingModule:
    ...
    def _on_event_selected(self, event):
        ...
        if event_id.startswith('country_'): return
    ...
    def _on_market_type_selected(self, event):
        ...
        if selection < 0 or selection >= len(self.available_markets): return
    ...
    def _display_market(self, market):
        ...
        self.tm_runners.update_flat(
            data=runners_data, id_getter=lambda r: str(r['id']), values_getter=lambda r: r['values'], tags_getter=lambda r: r['tags']
        )
    ...
    def _book_selected_runner(self):
        if not hasattr(self, '_context_menu_selection') or not self._context_menu_selection: return
    ...
    def _recalculate(self):
        if not self.selected_runners:
            self.selections_text.configure(state=tk.NORMAL); self.selections_text.delete('1.0', tk.END); self.selections_text.configure(state=tk.DISABLED)
            self.profit_label.configure(text="Profitto: -"); self.prob_label.configure(text="Probabilita Implicita: -"); self.place_btn.configure(state=tk.DISABLED)
            return
        
        self.selections_text.configure(state=tk.NORMAL); self.selections_text.delete('1.0', tk.END)
        ...
        try:
            ...
            for r in results:
                ...
                if bet_type == 'LAY':
                    text_lines.append(f"  Liability: {format_currency(r.get('liability', 0))}"); text_lines.append(f"  Se vince: {format_currency(r['profitIfWins'])}")
                else: text_lines.append(f"  Profitto se vince: {format_currency(r['profitIfWins'])}")
                text_lines.append("")
            ...
            else: self.profit_label.configure(text=f"Profitto Atteso: {format_currency(profit)}")
            self.prob_label.configure(text=f"Probabilita Implicita: {implied_prob:.1f}%")
            
            errors = validate_selections(results, bet_type)
            if not errors: self.place_btn.configure(state=tk.NORMAL)
            else:
                self.place_btn.configure(state=tk.DISABLED)
                self.selections_text.insert(tk.END, "\nErrori:\n" + "\n".join(errors))
            self.calculated_results = results
        except Exception as e:
            self.selections_text.insert('1.0', f"Errore calcolo: {e}"); self.profit_label.configure(text="Profitto: -"); self.place_btn.configure(state=tk.DISABLED)
        self.selections_text.configure(state=tk.DISABLED)
    ...
    def _open_dutching_window(self):
        ...
        market_data = {'marketId': self.current_market['marketId'], 'marketName': self.current_market.get('marketName', ''), 'eventName': self.current_event.get('name', '') if self.current_event else '', 'startTime': self.current_event.get('openDate', '')[:16] if self.current_event else '', 'status': self.market_status}
        open_dutching_window(parent=self.root, market_data=market_data, runners=runners, on_submit=self._place_dutching_orders, on_refresh=self._refresh_prices)
    ...
    def _place_dutching_orders(self, orders):
        ...
        if self.simulation_mode:
            for o in orders: self.db.add_simulated_bet(market_id=market_id, selection_id=o['selectionId'], runner_name=o['runnerName'], side=o['side'], price=o['price'], stake=o['size'])
        ...
        try:
            ...
            if result and result.get('status') == 'SUCCESS':
                messagebox.showinfo("Successo", f"Piazzati {len(orders)} ordini Dutching."); self._refresh_data()
            else: messagebox.showerror("Errore", f"Errore piazzamento: {result.get('errorCode', 'Errore sconosciuto') if result else 'Nessuna risposta'}")
        except Exception as e: messagebox.showerror("Errore", f"Errore: {str(e)}")
