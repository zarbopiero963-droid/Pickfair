...
class SelectionWoMHistory:
    ...
    def get_recent_from_snapshot(
        ...
        return [t for t in ticks_snapshot if ref_now - t.timestamp <= max_age_sec]
...
class WoMEngine:
    ...
    def _get_ticks_snapshot(
        ...
        now = time.time()
        return [t for t in ticks_snapshot if now - t.timestamp <= max_age_sec]
    ...
    def _calculate_wom_from_ticks(self, ticks: List[TickData]) -> Optional[float]:
        ...
        total_back_vol = sum(max(0.0, t.back_volume) for t in ticks)
        total_lay_vol = sum(max(0.0, t.lay_volume) for t in ticks)
    ...
    def get_mixed_suggestions(self, selections: List[Dict]) -> List[Dict]:
        ...
        for s in selections:
            ...
            if price > 1.0:
                valid_prices.append(1.0 / price)

        avg_prob = sum(valid_prices) / len(valid_prices) if valid_prices else 0.5
    ...
    def calculate_wom_window(self, selection_id: int, window_sec: float) -> float:
        ...
        ticks = self._get_ticks_snapshot(sel_id, max(0.1, self._safe_float(window_sec, 0.5)))
        wom = self._calculate_wom_from_ticks(ticks)
    ...
    def calculate_momentum(self, selection_id: int) -> float:
        ...
        return max(-1.0, min(1.0, momentum * 4.0))
    ...
    def get_time_window_signal(self, selection_id: int) -> Dict:
        ...
        return {
            "signal": signal,
            "strength": min(1.0, max(0.0, signal_strength * convergence)),
            "side": side,
            "reasoning": reasoning,
            "wom_data": {
