...
class TelegramModule: def _start_telegram_listener(self): """Start the Telegram listener background thread (Decoupled with EventBus).""" settings = self.db.get_telegram_settings() if not settings or not settings.get("api_id") or not settings.get("api_hash"): messagebox.showwarning( "Attenzione", "Configura e salva le credenziali Telegram prima di avviare il listener.", ) return
    ...
    try:
        ...
        for chat in self.db.get_telegram_chats():
            ...
            try:
                ...
                logger.warning(
                    f"[TelegramModule] chat_id non valido ignorato: {chat}"
                )
...
def _handle_telegram_signal(self, signal):
    ...
    if not getattr(self, "tg_auto_bet_var", None) or not self.tg_auto_bet_var.get():
        if getattr(self, "tg_confirm_var", None) and self.tg_confirm_var.get():
            msg = (
                f"Segnale ricevuto:\n"
                f"{selection_name}\n"
                f"Tipo: {action}\n"
    ...
    try:
        ...
        logger.error(f"[TelegramModule] Segnale ignorato: payload invalido ({e})")
        if hasattr(self.db, "save_received_signal"):
            self.db.save_received_signal(
                selection=selection_name,
                action=action,
    ...
    logger.info(
        f"[TelegramModule] Inoltro segnale all'OMS via RiskGate: {payload}"
    )
...
def _add_selected_available_chats(self):
    ...
    for item_id in selected:
        ...
        self.db.save_telegram_chat(chat_id=item_id, title=name, is_active=True)
...
def _add_signal_pattern(self):
    ...
    self.db.save_signal_pattern(pattern=pattern, label=label, enabled=True)
