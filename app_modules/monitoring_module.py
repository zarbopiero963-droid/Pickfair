...
class MonitoringModule:
    ...
    def _refresh_dashboard_tab(self):
        ...
        def create_stat_card(parent, title, value, subtitle, col):
            ...
            ctk.CTkLabel(card, text=title, font=('Segoe UI', 9), text_color=COLORS['text_secondary']).pack(pady=(10, 2))
            ctk.CTkLabel(card, text=value, font=FONTS['title'], text_color=COLORS['text_primary']).pack()
            ctk.CTkLabel(card, text=subtitle, font=('Segoe UI', 8), text_color=COLORS['text_tertiary']).pack(pady=(2, 10))
            return card
