...
from dutching_state import DutchingMode, DutchingState, RunnerState
from market_validator import MarketValidator
from theme import COLORS, FONTS
from trading_config import BOOK_BLOCK, BOOK_WARNING, MIN_STAKE
...
class DutchingConfirmationWindow:
    ...
    def __init__(
        ...
        self._simulation_var = tk.BooleanVar(
            value=getattr(self.state, "simulation_mode", False)
        )
    ...
    def _build_runner_table(self, parent):
        ...
        header_frame = ctk.CTkFrame(
            table_frame, fg_color=COLORS["bg_panel"], corner_radius=0
        )
    ...
    def _create_runner_row(self, idx: int, runner: RunnerState):
        ...
        name_label = ctk.CTkLabel(
            row_frame,
            text=runner.runner_name,
            font=FONTS["default"],
            text_color=(
                COLORS["text_primary"] if runner.included else COLORS["text_tertiary"]
            ),
            width=200,
            anchor="w",
        )
    ...
    def _create_mixed_runner_rows(self, idx: int, runner: RunnerState):
        ...
        # === RIGA BACK (blu) ===
        back_frame = ctk.CTkFrame(container, fg_color=COLORS["back"], corner_radius=4)
        ...
        back_name = ctk.CTkLabel(
            back_frame,
            text=f"{runner.runner_name} - BACK",
            font=FONTS["default"],
            text_color=COLORS["bg_dark"],
            width=220,
            anchor="w",
        ...
        # === RIGA LAY (rosa) ===
        lay_frame = ctk.CTkFrame(container, fg_color=COLORS["lay"], corner_radius=4)
    ...
    def _toggle_mixed_mode(self):
        ...
        if self._mixed_mode_var.get():
            self.mixed_btn.configure(fg_color=COLORS["warning"], text="Mixed: ON")
        else:
            self.mixed_btn.configure(
                fg_color=COLORS["button_secondary"], text="Mixed Mode"
            )
    ...
    def _toggle_ai_mode(self):
        ...
        if self._ai_mode_var.get():
            self.ai_btn.configure(fg_color=COLORS["profit"], text="AI: ON")
            self._mixed_mode_var.set(False)
            self.mixed_btn.configure(
                fg_color=COLORS["button_secondary"], text="Mixed Mode", state="disabled"
            )
        else:
            self.ai_btn.configure(fg_color=COLORS["button_secondary"], text="AI Auto")
    ...
    def _show_market_warning(self):
        ...
        if not hasattr(self, "_market_warning_label"):
            self._market_warning_label = ctk.CTkLabel(
                self.window, text="", font=FONTS["small"], text_color="#FF5555"
            )
    ...
    def _build_footer(self, parent):
        ...
        offset_label = ctk.CTkLabel(
            controls_frame, text="Global Offset:", text_color=COLORS["text_primary"]
        )
        offset_label.pack(side=tk.LEFT, padx=(0, 5))

        offset_entry = ctk.CTkEntry(
            controls_frame,
            textvariable=self._global_offset_var,
            width=50,
            font=FONTS["small"],
            fg_color=COLORS["bg_surface"],
            text_color=COLORS["text_primary"],
    ...
    def _on_mode_change(self):
        ...
        self.state.mode = (
            DutchingMode.STAKE_AVAILABLE
            if mode == "stake"
            else DutchingMode.REQUIRED_PROFIT
        )
        self._recalculate()
    ...
    def _on_submit(self):
        ...
        if not orders:
            messagebox.showwarning(
                "Attenzione", "Nessun ordine da piazzare.", parent=self.window
            )
            return
        ...
        errors = validate_selections(
            [
                {"runnerName": o["runnerName"], "stake": o["size"], "price": o["price"]}
                for o in orders
            ],
            bet_type="BACK",
        )
        ...
        if messagebox.askyesno("Conferma", msg, parent=self.window):
            ...
            if sim_mode:
                ...
                print(
                    f"[Dutching] SIMULATION: {len(orders)} ordini simulati per €{total:.2f}"
                )
                for o in orders:
                    print(
                        f"  [SIM] {o['side']} {o['runnerName']} @ {o['price']} €{o['size']:.2f}"
                    )
                messagebox.showinfo(
                    "Simulazione",
                ...
                if auto_green:
                    print(
                        "[Dutching] Auto-Green attivo - monitoring P&L per green-up automatico"
                    )
    ...
    def _recalculate(self):
        ...
        try:
            ...
            if mode == "stake":
                ...
                total_stake = (
                    self.state.target_profit / (avg_odds - 1) * len(selections)
                )
    ...
    def _find_best_odds(self):
        ...
        for runner in self.state.runners:
            ...
            if runner.effective_type == "BACK":
                if (
                    best_back is None
                    or runner.effective_odds > best_back.effective_odds
                ):
                    best_back = runner
            else:  # LAY
            ...
            if "odds_entry" in widgets:
                try:
                    if is_best and runner.included:
                        widgets["odds_entry"].configure(
                            border_color=COLORS["profit"], border_width=2
                        )
                    else:
                        widgets["odds_entry"].configure(
                            border_color=COLORS["border"], border_width=1
    ...
    def _update_pnl_preview(self):
        ...
        self.pnl_preview_label.configure(
            text=f"Guaranteed: {pnl_text} | Loss: -€{total_stake:.2f}", text_color=color
        )
    ...
    def _update_totals(self, total_stake: float, book_value: float):
        ...
        if book_value > BOOK_BLOCK:
            ...
            book_text = f"Book: {book_value:.1f}%"
            book_color = (
                COLORS["profit"] if book_value < 100 else COLORS["text_primary"]
            )
            can_submit = has_valid_selections
        else:
            book_text = "Book: --%"
...
def open_dutching_window(
    ...
    window = DutchingConfirmationWindow(
        parent=parent, state=state, on_submit=on_submit, on_refresh_odds=on_refresh
    )
