...
class AutoThrottle:
    ...
    def update(self) -> LoadLevel:
        ...
        with self._lock:
            ...
            if new_level != self._current_level:
                ...
                logger.info(
                    f"[THROTTLE] Level changed: {old_level.value} -> {new_level.value}"
                )
    ...
    def _apply_hysteresis(self, raw_level: LoadLevel, now: float) -> LoadLevel:
        ...
        if self._pending_level != raw_level:
            ...
            logger.debug(
                f"[THROTTLE] Pending transition: {self._current_level.value} -> {raw_level.value}"
            )
            return self._current_level
        ...
        if elapsed >= required_time:
            ...
            self._pending_since = 0.0
            logger.debug(
                f"[THROTTLE] Hysteresis passed ({elapsed:.1f}s >= {required_time}s)"
            )
            return raw_level
    ...
    def _value_to_level(
        self, value: float, thresholds: Dict[LoadLevel, float]
    ) -> LoadLevel:
    ...
    def get_stats(self) -> Dict:
        ...
        if self._pending_level is not None:
            ...
            going_up = level_order.index(self._pending_level) > level_order.index(
                self._current_level
            )
