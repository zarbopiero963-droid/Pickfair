...
Il rate-limiting applicativo è gestito dai timer UI / executor / scheduler.
Il blocco chiamate in caso di errore è gestito da circuit breaker e OMS.
Manteniamo questo file solo per retrocompatibilità con codice legacy che potrebbe ancora importarlo o chiamarne alcuni metodi. """
import logging
logger = logging.getLogger("AutoThrottle")
class AutoThrottle: def init(self, *args, **kwargs): logger.warning( "[DEPRECATED] AutoThrottle istanziato. " "Usare RiskMiddleware / Executor / OMS." ) self._last_rate = 0.0 self._blocked = False
def wait(self):
