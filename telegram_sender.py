...
class AdaptiveRateLimiter:
    ...
    def record_flood_wait(self, wait_seconds: int):
        with self._lock:
            self.consecutive_successes = 0
            self.current_delay = max(self.current_delay, min(wait_seconds / 10.0, self.base_delay * 10))

    def get_stats(self):
        return {'current_delay': self.current_delay, 'consecutive_successes': self.consecutive_successes}
...
class TelegramSender:
    ...
    async def send_message(self, chat_id: str, text: str, max_retries: int = 3) -> SendResult:
        result = SendResult()
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if 'floodwait' in error_str or 'flood' in error_str:
                    ...
                    logger.warning(f"[TG_SENDER] FloodWait {wait_seconds}s. Safe sleep: {safe_wait}s. Attempt {attempt+1}/{max_retries}")
    ...
    def send_message_sync(self, chat_id: str, text: str, max_retries: int = 3) -> SendResult:
    ...
    def queue_message(self, chat_id: str, text: str, max_retries: int = 3, callback: Optional[Callable] = None):
        msg = QueuedMessage(chat_id=chat_id, text=text, max_retries=max_retries, callback=callback)
        self._queue.put(msg)
