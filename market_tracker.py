...
class DeltaDetector:
    ...
    def has_changed(
        ...
        with self._lock:
            ...
            price_changed = back_diff >= self.min_price_change or lay_diff >= self.min_price_change
            volume_changed = back_vol_diff >= self.min_volume_change or lay_vol_diff >= self.min_volume_change
            ...
            if price_changed or volume_changed:
                ...
                return True, f"Volume: BACK Δ{back_vol_diff:.1f}, LAY Δ{lay_vol_diff:.1f}"
...
class MarketTracker:
    ...
    def get_market_book(self, market_id: str, force_refresh: bool = False) -> Optional[Dict]:
        """
    ...
    def get_changed_prices(self, market_id: str) -> Dict[int, Dict]:
        ...
        for sel_id, price_data in all_prices.items():
            has_changed, reason = self.delta.has_changed(
                market_id, sel_id,
                price_data['back'], price_data['lay'],
                price_data['back_size'], price_data['lay_size']
            )
            if has_changed:
