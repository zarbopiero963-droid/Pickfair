...
class PickfairApp(
    ...
    def _init_window(self):
        ...
        if screen_width <= 1366 or screen_height <= 768:
            self.root.after(100, lambda: getattr(self, "_try_maximize", lambda: None)())
    ...
    def _bootstrap_ui(self):
        if hasattr(self, "_create_menu"): self._create_menu()
        if hasattr(self, "_create_main_layout"): self._create_main_layout()

        if hasattr(self, "events_tree"): self.tm_events = TreeManager(self.events_tree)
        if hasattr(self, "runners_tree"): self.tm_runners = TreeManager(self.runners_tree)
        if hasattr(self, "placed_bets_tree"): self.tm_placed_bets = TreeManager(self.placed_bets_tree)
        if hasattr(self, "market_cashout_tree"): self.tm_cashout = TreeManager(self.market_cashout_tree)

        if hasattr(self, "_load_settings"): self._load_settings()
        if hasattr(self, "_configure_styles"): self._configure_styles()
    ...
    def _wire_events(self):
        ...
        if hasattr(self, "_update_telegram_status"):
            self.bus.subscribe("TELEGRAM_STATUS", lambda d: self.uiq.post(
                lambda: self._update_telegram_status(d.get("status"), d.get("message"))
            ))
        ...
        # 4. OMS Callbacks (Risposte da Betfair)
        if hasattr(self, "_on_engine_success"):
            self.bus.subscribe("QUICK_BET_SUCCESS", lambda d: self.uiq.post(self._on_engine_success, d))
            self.bus.subscribe("DUTCHING_SUCCESS", lambda d: self.uiq.post(self._on_engine_success, d))

        if hasattr(self, "_on_engine_error"):
            self.bus.subscribe("QUICK_BET_FAILED", lambda e: self.uiq.post(self._on_engine_error, e))
            self.bus.subscribe("DUTCHING_FAILED", lambda e: self.uiq.post(self._on_engine_error, e))

        if hasattr(self, "_on_cashout_success"):
            self.bus.subscribe("CASHOUT_SUCCESS", lambda d: self.uiq.post(self._on_cashout_success, d))

        if hasattr(self, "_on_cashout_failed"):
            self.bus.subscribe("CASHOUT_FAILED", lambda e: self.uiq.post(self._on_cashout_failed, e))
