...
class PluginManager:
                    ...
                    if isinstance(func.value, ast.Name):
                        ...
                        if base_module in self.BLOCKED_CALLS:
                            if method_name in self.BLOCKED_CALLS[base_module]:
                                return (
                                    False,
                                    f"Chiamata vietata: {obj_name}.{method_name}()",
                                )
                        ...
                        if base_module in self.BLOCKED_MODULES:
                            return (
                                False,
                                f"Chiamata a modulo vietato: {obj_name}.{method_name}()",
                            )
    ...
    def load_plugin(self, filepath: str, enable: bool = True) -> tuple:
        ...
        if not req_ok:
            info = PluginInfo(
                name=plugin_name, enabled=False, error=req_msg, path=filepath
            )
        ...
        try:
            ...
            if thread.is_alive():
                return (
                    ...
                    "Plugin timeout durante il caricamento",
                )
            ...
            info = PluginInfo(
                name=plugin_name, enabled=False, error=str(e), path=filepath
            )
    ...
    def enable_plugin(self, plugin_name: str) -> tuple:
        ...
        with self._lock:
            ...
            if info.module and hasattr(info.module, "register"):
                self._run_plugin_safe(
                    lambda: info.module.register(self.app), plugin_name
                )
    ...
    def _run_plugin_safe(
        self, func: Callable, plugin_name: str, *args, **kwargs
    ) -> Any:
        """Execute a plugin function safely via ThreadPoolExecutor."""
        return self.plugin_runner.run(plugin_name, func, *args, **kwargs)

    def register_hook(
        self, hook_name: str, callback: Callable, plugin_name: str = None
    ):
