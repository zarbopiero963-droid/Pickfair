...
class TradingEngine:
    def __init__(self, bus, db, client_getter, executor):
        self.bus = bus
        self.db = db
        self.client_getter = client_getter  # Lambda per ottenere il client sempre aggiornato
            ...
            if sim_mode:
                try:
                    ...
                    if liability > current_balance:
                        self.bus.publish("QUICK_BET_FAILED", f"Saldo virtuale insufficiente.\nSaldo: {current_balance}\nRichiesto: {liability}")
                        return
                    ...
                    self.bus.publish("QUICK_BET_SUCCESS", {
                        "runner_name": runner['runnerName'], "price": price, "stake": stake, "new_balance": new_balance, "sim": True
                    })
                ...
                try:
                    ...
                    if result.get('status') == 'SUCCESS':
                        ...
                        self.bus.publish("QUICK_BET_SUCCESS", {
                            "runner_name": runner['runnerName'], "price": price, "matched": matched, "sim": False
                        })
                    else:
        ...
        def task():
            ...
            if sim_mode:
                try:
                    ...
                    selections_info = [{'name': r.get('runnerName', 'Unknown'), 'price': r['price'], 'stake': r['stake']} for r in results]
                    ...
                    self.bus.publish("DUTCHING_SUCCESS", {"sim": True, "total_stake": total_stake, "profit": results[0]['profitIfWins'] if results else 0, "new_balance": new_balance})
                except Exception as e:
            ...
            try:
                    ...
                    if book and book.get('runners'):
                        for runner in book['runners']:
                            ...
                            if bet_type == 'BACK':
                                ...
                                if backs: current_prices[sel_id] = backs[0].get('price', 1.01)
                            else:
                                lays = ex.get('availableToLay', [])
                                if lays: current_prices[sel_id] = lays[0].get('price', 1000)
                ...
                if result.get('status') == 'SUCCESS':
                    bet_status = 'MATCHED' if all_matched else 'PARTIALLY_MATCHED' if any_matched else 'PENDING'
                ...
                for i, r in enumerate(results):
                    ...
                    selections_with_names.append({
                        'runnerName': r.get('runnerName', 'Unknown'), 'selectionId': r['selectionId'],
                        'price': r['price'], 'stake': r['stake'], 'sizeMatched': report.get('sizeMatched', 0),
                        'betId': report.get('betId'), 'instructionStatus': report.get('status', 'UNKNOWN')
                    })
                
                self.db.save_bet(
                    event['name'], market['marketId'], market.get('marketName', ''), bet_type,
                    selections_with_names, total_stake, results[0]['profitIfWins'] if results else 0, bet_status
                )
    ...
    def _handle_cashout(self, payload):
        ...
        def task():
            try:
                ...
                result = client.execute_cashout(market_id, selection_id, side, stake, price)
                
                if result.get('status') == 'SUCCESS':
                    self.db.save_cashout_transaction(
                        market_id=market_id, selection_id=selection_id, original_bet_id=bet_id,
                        cashout_bet_id=result.get('betId'), original_side=original_pos['side'],
                        original_stake=original_pos['stake'], original_price=original_pos['price'],
                        cashout_side=side, cashout_stake=stake,
                        cashout_price=result.get('averagePriceMatched') or price, profit_loss=green_up
                    )
                    self.bus.publish("CASHOUT_SUCCESS", {"green_up": green_up})
                elif result.get('status') == 'ERROR':
                    self.bus.publish("CASHOUT_FAILED", f"Errore: {result.get('error', 'Sconosciuto')}")
                else:
